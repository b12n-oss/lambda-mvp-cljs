(ns script.bench-run
  ...
  the Jolt/jank siblings' own numbers on their face, since this
  project deploys to AWS's MANAGED nodejs24.x runtime, not a custom
...
(defn- invoke-sample! []
  (let [out-file (str (System/getProperty "java.io.tmpdir")
                      "/lambda-mvp-cljs-bench-" (System/nanoTime) ".json")
        {:keys [exit out err]}
  ...
  (let [cold (invoke-sample!)]
    ...
    (let [warm (mapv (fn [_] (invoke-sample!)) (range warm-samples))]
      ...
      (when (some :init-duration-ms warm)
        (println "lambda-mvp-cljs: WARNING -- a 'warm' sample for" tier
